    ...
    <div className="rounded-lg border border-border bg-card px-4 py-3 shadow-lg text-sm">
      ...
      {payload.map((entry) => (
        <div key={entry.name} className="flex items-center gap-2 py-0.5">
          <span className="inline-block h-2 w-2 rounded-full" style={{ backgroundColor: entry.color }} aria-hidden="true" />
    ...
    <Card className="border-border/60">
      ...
      <CardContent>
        {!hasData ? (
          ...
          <div role="img" aria-label="Line chart comparing monthly income and outcome values.">
